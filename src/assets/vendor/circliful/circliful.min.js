    ...
    t.fn.circliful = function(e, o) {
        ...
        return this.each(function() {
            ...
            if (r.start > 0 && r.target > 0 && (c = r.start / (r.target / 100), p = r.target / 100), 1 == r.animation) var m = window.setInterval(function() {
                u >= 3.6 * c ? (window.clearInterval(m), h = 1) : (u += y, C += p), u / 3.6 >= c && 1 == h && (u = 3.6 * c), C > r.target && 1 == h && (C = r.target), g.attr("stroke-dasharray", u + ", 20000"), 1 == r.showPercent ? k.text(parseInt(u / 360 * 100) + "%") : k.text(C)
            }.bind(g), f);
            else g.attr("stroke-dasharray", 3.6 * c + ", 20000"), 1 == r.showPercent ? k.text(c + "%") : k.text(r.target)
        })
    }
}(jQuery);